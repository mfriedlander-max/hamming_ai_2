...
function NewIterationPageInner() {
  ...
  // Load prompt from selected iteration's latest version
  useEffect(() => {
    if (promptSource === "continue" && selectedIteration) {
      // Fetch the latest version's content for the selected iteration
      getLatestVersion(selectedIteration).then((latestVersion) => {
        if (latestVersion) {
          setSystemPrompt(latestVersion.content);
        } else {
          // Fallback to project's systemPrompt if no versions exist
